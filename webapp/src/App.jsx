...
function BottomNav({ cartCount, isAdmin }) {
  ...
  const tabs = [
    { to: '/',         icon: Home,     label: t('nav_home')    || 'Дом' },
    { to: '/catalog',  icon: Package,  label: t('nav_catalog') || 'Каталог' },
    { to: '/checkout', icon: Sparkles, label: 'Заказ' },
    { to: '/profile',  icon: User,     label: t('nav_profile') || 'Профиль' },
  ];
  if (isAdmin) tabs.push({ to: '/admin', icon: ShieldCheck, label: t('nav_admin') || 'Админ' });
  ...
  return (
    <nav className="bottom-nav" data-testid="bottom-nav">
      {tabs.map((tab) => {
        ...
        return (
          <Link key={tab.to} to={tab.to} className={`nav-item ${active ? 'active' : ''}`} data-testid={`nav-${tab.to.replace('/', '') || 'home'}`}>
...
function DesktopHeader({ isAdmin }) {
  ...
  const navLinks = [
    { to: '/',         label: t('nav_home')    || 'Атлас' },
    { to: '/catalog',  label: t('nav_catalog') || 'Каталог' },
    { to: '/checkout', label: 'Заказать' },
    { to: '/profile',  label: t('nav_profile') || 'Профиль' },
  ];
  if (isAdmin) navLinks.push({ to: '/admin', label: t('nav_admin') || 'Админ' });

  return (
    <header className="desktop-header" data-testid="desktop-header">
      <Link to="/" className="header-logo" data-testid="logo-link">
        <span className="logo-title">fabrika<span className="dot">.</span>flo</span>
      ...
      <nav className="header-nav">
        {navLinks.map((link) => {
          ...
          return (
            <Link key={link.to} to={link.to} className={`nav-link ${active ? 'active' : ''}`} data-testid={`header-nav-${link.to.replace('/', '') || 'home'}`}>
              {link.label}
            </Link>
          );
        })}
    ...
    <div className="container">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem', paddingTop: '1rem' }}>
        ...
        <div className="responsive-products-grid">
          {[1,2,3,4].map(i => <div key={i} className="skeleton" style={{ aspectRatio: '4/5' }} />)}
        </div>
      </div>
    </div>
  );
...
function BotanicalSvg() {
  ...
  return (
    <svg className="splash-svg" viewBox="0 0 220 220" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
      {/* Outer petals */}
      <path className="stroke" d="M110 40 C 80 55, 65 85, 78 118 C 55 118, 40 140, 55 165 C 80 178, 108 168, 115 145 C 135 165, 168 158, 176 132 C 190 106, 172 82, 150 82 C 158 60, 140 42, 110 40 Z" />
      {/* Middle ring */}
      <path className="stroke accent-stroke" d="M100 78 C 88 82, 82 96, 90 108 C 78 112, 76 128, 88 138 C 100 145, 118 138, 122 125 C 138 138, 158 128, 158 112 C 158 95, 140 88, 128 92 C 130 76, 116 72, 100 78 Z" />
      {/* Center */}
      <path className="stroke" d="M110 110 C 102 112, 100 122, 108 128 C 116 132, 128 128, 128 118 C 128 108, 118 106, 110 110 Z" />
      {/* Leaves */}
      <path className="stroke accent-stroke" d="M55 175 C 40 190, 30 200, 20 205" />
      <path className="stroke accent-stroke" d="M175 155 C 190 170, 198 185, 200 200" />
...
function SplashScreen({ visible }) {
  ...
  return (
    <div className="splash-screen" style={{ opacity: visible ? 1 : 0, pointerEvents: visible ? 'auto' : 'none' }} data-testid="splash-screen">
      <div className="splash-inner">
        ...
        <div className="splash-brand">fabrika<span style={{color:'var(--champagne-lo)'}}>.</span>flo</div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initTask = (initData || user || localStorage.getItem('auth_token'))
  ...
  const loadProfile = async () => {
    try { const data = await api.getProfile(); setProfile(data); }
    catch (e) { console.error(e); }
  };

  const updateCartCount = () => { setCartCount(0); };
