...
export default function Catalog() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([c, p]) => {
        setCategories([{ name: 'Все', slug: '' }, { name: 'Акции', slug: 'sale' }, ...c]);
        setProducts(p);
      })
      .catch(console.error)
                          ...
                          <button
                            ...
                            onClick={async (e) => {
                              ...
                              try {
                                ...
                                const updated = await api.getProducts(activeCategory ? { category: activeCategory } : {});
                                setProducts(updated); haptic.success();
                              } catch (err) { console.error(err); }
                            }}
                            title={p.in_stock ? 'Скрыть' : 'Показать'}
                          >
                            {p.in_stock ? <Eye size={13} /> : <EyeOff size={13} />}
                          </button>
