...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (profile.address_lat && profile.address_lng) {
      setMapPosition({ lat: profile.address_lat, lng: profile.address_lng });
    } else {
  ...
  const saveProfileData = async () => {
    try {
      ...
      const updatedProfile = await updateProfileMutation.mutateAsync({ name: editName });
      setEditName(updatedProfile.name || editName);
    ...
    <div className="container page-transition" style={{ paddingBottom: '6rem' }} data-testid="profile-page">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem', paddingTop: '0.5rem' }}>
        ...
        >
          {profile.photo_url ? (
            <img 
              src={profile.photo_url.startsWith('/') ? `${API_URL.replace('/api', '')}${profile.photo_url}` : profile.photo_url} 
              alt="Avatar" 
      ...
      <div className="glass-card" style={{ 
        ...
        <div className="flex-between" style={{ marginBottom: '1rem' }}>
          <h3 style={{ color: 'var(--wine)', display: 'flex', alignItems: 'center', gap: '0.5rem', fontFamily: 'var(--font-display)', fontStyle: 'italic', fontWeight: 400 }}>
            <GiftIcon width={18} height={18} strokeWidth={1.5} /> Партнёрская программа
          </h3>
        </div>
        ...
        {profile?.is_partner ? (
          ...
          <button className="primary" style={{ width: '100%', padding: '1rem' }} onClick={copyRefLink}>
            <CopyIcon width={18} height={18} /> Скопировать ссылку
          </button>
        )}
