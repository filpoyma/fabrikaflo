...
import { Upload, X, MapPin, Check, ArrowRight } from 'lucide-react';
...
export default function Checkout() {
  ...
  useEffect(() => {
    api.getProfile().then(p => {
      setProfile(p);
      setForm(f => ({ ...f, recipientPhone: p.phone || '', deliveryAddress: p.address || '' }));
      if (p.address_lat && p.address_lng) setMapPosition({ lat: parseFloat(p.address_lat), lng: parseFloat(p.address_lng) });
    }).catch(console.error);
  }, []);
      ...
      <form onSubmit={submit}>
          ...
          {form.deliveryType === 'DELIVERY' ? (
            <>
              ...
              <button type="button" className="secondary" style={{ width: '100%', padding: '0.7rem', fontSize: '0.75rem', letterSpacing: '0.2em', marginBottom: '1rem' }} onClick={locateMe}>
                <MapPin size={14} strokeWidth={1.5} /> Моя геопозиция
              </button>
              <textarea
        ...
        <div style={{ padding: '2rem 0', borderTop: '1px solid var(--line)' }}>
          <button type="submit" className="primary" style={{ width: '100%', padding: '1.1rem', fontSize: '0.85rem', letterSpacing: '0.22em' }} disabled={submitting || uploadingPhoto} data-testid="submit-request">
            {submitting ? 'Отправка…' : <>Отправить флористу <ArrowRight size={14} strokeWidth={1.6} /></>}
          </button>
