...
import { ArrowLeft, Minus, Plus, Edit2, Check, Upload, ArrowRight } from 'lucide-react';
import { api } from '../api';
...
export default function Product({ updateCart }) {
    ...
    api.getProduct(id)
      .then(async (p) => {
        ...
        try {
          ...
          setSimilarProducts(all.filter(x => x.id !== p.id && x.category_slug === p.category_slug).slice(0, 4));
  ...
  const handleSave = async () => {
    try {
      ...
      setProduct(updated); setIsEditing(false); haptic.success();
      ...
      <div style={{
        ...
        {isEditing ? (
          <>
            ...
            >
              <Upload size={30} color="var(--ivory)" strokeWidth={1.4} />
            </div>
        ...
        <div style={{ paddingTop: '2rem' }}>
          <div className="flex-between" style={{ alignItems: 'flex-start', marginBottom: '1rem' }}>
            ...
            {canEditProduct && (
              <button className="icon-btn" onClick={isEditing ? handleSave : toggleEdit} disabled={saving} style={{ marginLeft: '0.8rem' }} data-testid="edit-toggle">
                {isEditing ? <Check size={16} strokeWidth={1.5} /> : <Edit2 size={15} strokeWidth={1.5} />}
              </button>
            )}
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', padding: '1.4rem 0', borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)', marginBottom: '1.6rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', border: '1px solid var(--line)', borderRadius: '999px', padding: '0.15rem' }}>
              <button className="icon-btn" style={{ width: 32, height: 32 }} onClick={() => setQty(Math.max(1, qty - 1))} data-testid="qty-minus"><Minus size={13} strokeWidth={1.5} /></button>
              <span style={{ fontFamily: 'var(--font-sans)', width: 22, textAlign: 'center' }}>{qty}</span>
              <button className="icon-btn" style={{ width: 32, height: 32 }} onClick={() => setQty(qty + 1)} data-testid="qty-plus"><Plus size={13} strokeWidth={1.5} /></button>
            </div>
          </div>
          ...
          >
            Заказать аналогичный <ArrowRight size={14} strokeWidth={1.6} />
          </button>
        </div>
